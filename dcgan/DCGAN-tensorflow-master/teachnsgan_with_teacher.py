...
class NSGAN(object):
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, int(batch_idxs)):
        ...
        if counter % self.plot_every == 0:    
          ...
          if config.mode == "training":
            self.score_training(eval_samples)
          else:
            self.score_refinement(batch_z, batch_images, counter, epoch, idx)

      if config.save_model:
        if config.mode == "training" and ((epoch % 4 == 0) or epoch == 1 or epoch == 2):
          print("Saving Model")
  ...
  def score_training(self, eval_samples):

    ## Get Quantitative Comparisons
    eval_z = np.random.uniform(-1, 1, [self.eval_size, self.z_dim]).astype(np.float32)
    eval_images = self.data_X[idx*config.batch_size:(idx+1)*config.batch_size]
    ...
    # fid_score_mnist = self.fid_eval_score.eval({self.z: eval_z, self.inputs: batch_images})
    self.fid_score_mnist = self.fid_eval_score.eval({self.evaluator: eval_samples, self.inputs: batch_images})              
    print("FID Score of MNIST default: ", self.fid_score_mnist)
  ...
  def evaluator(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      ...
      net = tf.nn.relu(bn(linear(z, 1024, scope='g_fc1'), is_training=False, scope='g_bn1'))
      net = tf.nn.relu(bn(linear(net, 128 * 7 * 7, scope='g_fc2'), is_training=False, scope='g_bn2'))
      net = tf.reshape(net, [self.eval_size, 7, 7, 128])
      net = tf.nn.relu(
          bn(deconv2d(net, [self.eval_size, 14, 14, 64], 4, 4, 2, 2, name='g_dc3'), is_training=False,
             scope='g_bn3'))

      out = tf.nn.tanh(deconv2d(net, [self.eval_size, 28, 28, 1], 4, 4, 2, 2, name='g_dc4'))
