...
from wgan_cs import WGAN
from utils_image import pp, visualize, to_json, show_all_variables, make_folders
...
flags.DEFINE_string("refiner_name", "default", "refiner options: default | scalized | mcts | rollout | gpurollout")

flags.DEFINE_float("D_LR", 1.0, "Multiplicative Factor of LR of D[1]")
flags.DEFINE_integer("G_LR", 5, "Multiplicative Factor of LR of G[1]")
...
flags.DEFINE_string("mode", "refinement", "mode options: training | refinement | testing | denoising ")
...
flags.DEFINE_integer("load_epoch", 27000, "Epoch to load from for refinement")
flags.DEFINE_string("load_model_dir", "checkpoints/mnist/epoch_25_refiner_default_rollout_method_momentum_rollout_steps_10_rollout_rate_5.00000/", "directory to load model from")
...
flags.DEFINE_integer("rollout_steps", 10, "Roll Out Steps. [100]")
flags.DEFINE_float("rollout_rate", 0.1, "Roll Out Rate [50]")
flags.DEFINE_string("rollout_method", "momentum", "Rollout Method: sgd | momentum")
flags.DEFINE_boolean("imbalance", False, "True for creating imbalanced dataset")
flags.DEFINE_integer("collab_layer", 3, 
                    "Layer to collaborate. 0 -> z, 4 -> logit, 5 -> x")
flags.DEFINE_float("lambd", 0.10, "Gradient Penalty Weight  [0.1]")
...
load_iter: If not training, the iterations/epoch to load the saved model from
...
# checkpoints/mnist/epoch_21_refiner_default_rollout_method_sgd_rollout_steps_100_rollout_rate_10.00000/mnist_64_28_28/
# checkpoints/mnist/epoch_5_refiner_default_rollout_method_sgd_rollout_steps_100_rollout_rate_10.00000/mnist_64_28_28/
# checkpoints/mnist/epoch_22_refiner_default_rollout_method_sgd_rollout_steps_1_rollout_rate_1.00000/mnist_64_28_28/
# checkpoints/mnist/epoch_5_refiner_default_rollout_method_momentum_rollout_steps_100_rollout_rate_55.00000/mnist_64_28_28/
...
def main(_):
  ...
  if FLAGS.mode == 'training':
    FLAGS.refiner_name = 'default'
  else:
    FLAGS.refiner_name = 'gpurollout'
  ...
  with tf.Session(config=run_config) as sess:
    wgan = WGAN(
        ...
        mode=FLAGS.mode,
        collab_layer=FLAGS.collab_layer,
        lambd=FLAGS.lambd,
        config=FLAGS)
    ...
    if FLAGS.train:
      ...
      for i in range(FLAGS.num_it):
        inc_score, fid_score = wgan.train(FLAGS)
      ...
      print("Avg FID Score")
      print(ave_fid_score)
      wgan.save_scores(ave_inc_score, ave_fid_score, config=FLAGS, num_it=FLAGS.num_it)

    else:
      if not nsgan.load(FLAGS.checkpoint_dir)[0]:
